...
$sql = sprintf("select username
		      ...
		      ,ARRAY_TO_STRING(
			ARRAY(
			    SELECT approval.username FROM approval
			    WHERE approval.qid=assignment_gmaptile_13.gid
			    ORDER BY approval.username
			    ), '<br>') AS approver
                from assignment_gmaptile_13
                left join target_tiles_gmaptile_13 on assignment_gmaptile_13.gid = target_tiles_gmaptile_13.gid 
                where end_ts is not null 
                  and end_ts > '2100-01-01'
				  and assignment_gmaptile_13.username = '" . $username . "'
                order by end_ts desc;");
  if (!($rs = pg_exec($sql))) {
    die;
  }
	...
	$tiles='<a href="javascript:void(0);" onClick="writemapthumb('.$thumbsize.','.$lonmin.','.$latmin.','.$lonmax.','.$latmax.','.$i.','."'thumb_unfinished'".');" id="thumb_unfinished'.$i.'">Thumbnail</a>';
	...
	$html_unfinished = $html_unfinished . "<tr align='center'><td>" . $row["username"] . "</td>"
	    . "<td>" . $row["gid"] . "</td>"
	    . "<td>" . $tiles . "</td>"
	    . "<td><a href='wms.tms.parallel.v2.php?lonmin=". $row["lonmin"] ."&latmin=". $row["latmin"] ."&lonmax=". $row["lonmax"] ."&latmax=". $row["latmax"] ."'>Revise</a>" . "</td>"
...
<body>
<a href="http://guam.csis.u-tokyo.ac.jp/verify/assignregion.php">Back to assignment</a>
